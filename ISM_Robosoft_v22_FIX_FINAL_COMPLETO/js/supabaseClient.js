...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async listTeams() {
      const { data, error } = await assertClient().rpc('admin_list_teams_full_v22');
      if (error) throw error;
      return (data || []).map(t => ({
        ...
        teachers: t.teachers || [], students: Array.isArray(t.student_ids) ? t.student_ids.length : (t.students || 0), courses: t.courses || [], divisions: t.divisions || [], logo_url: t.logo_url || '', members: t.members || []
      }));
    },
    async saveTeam(payload) {
      const { data, error } = await assertClient().rpc('admin_save_team_full_v22', {
    ...
    async deleteTeam(id) {
      const { error } = await assertClient().rpc('admin_delete_team_v22', { p_team_id: String(id) });
      if (error) throw error;
    },
    ...
    async saveInventoryCondition(name, color = '#64748b', options = {}) {
      ...
      const { data, error } = await client.rpc('inventory_condition_save_v22', {
        p_name: cleanName,
        p_color: cleanColor,
    ...
    async updateInventoryAsset(id, payload) {
      ...
      const { error } = await assertClient().rpc('admin_update_inventory_asset_v22', {
        p_asset_id: assetId,
        p_name: payload.item,
    ...
    async bulkUpdateInventoryAssets(ids, changes) {
      const { error } = await assertClient().rpc('bulk_update_inventory_assets_v22', {
        p_asset_ids: ids || [],
    ...
    async createInventoryAsset(payload) {
      const { data, error } = await assertClient().rpc('admin_create_inventory_asset_v22', {
        p_name: payload.item,
        p_category: payload.category || payload.type || 'General',
        p_asset_code: payload.code || null,
        p_serial_number: payload.serial || null,
