...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async listInventoryConditions() {
      ...
      const rpc = await client.rpc('list_inventory_conditions_v20');
    ...
    async saveInventoryCondition(name, color = '#64748b', options = {}) {
      ...
      const { data, error } = await client.rpc('inventory_condition_save_v20', {
        p_name: cleanName,
        p_color: cleanColor,
    ...
    async updateInventoryAsset(id, payload) {
      ...
      const { error } = await assertClient().rpc('admin_update_inventory_asset_v20', {
        p_asset_id: assetId,
        p_name: payload.item,
    ...
    async bulkUpdateInventoryAssets(ids, changes) {
      const { error } = await assertClient().rpc('bulk_update_inventory_assets_v20', {
        p_asset_ids: ids || [],
    ...
    async createInventoryAsset(payload) {
      const { data, error } = await assertClient().rpc('admin_create_inventory_asset_v20', {
        p_name: payload.item,
        p_category: payload.category || payload.type || 'General',
        p_asset_code: payload.code || null,
        p_serial_number: payload.serial || null,
