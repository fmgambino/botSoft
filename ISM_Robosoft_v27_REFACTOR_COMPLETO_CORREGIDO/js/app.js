...
function viewRecord(type, row) {
  ...
  const labels = type === 'user'
    ? { name:'Nombre completo', email:'Email', role:'Rol', role_code:'Código de rol', whatsapp:'WhatsApp', dni:'DNI', status:'Estado', birth_date:'Fecha de nacimiento', title:'Título / Certificados', student_course_name:'Curso', student_division_name:'División', course:'Curso', division:'División', student_names:'Integrantes', teachers:'Mentores', courses:'Cursos', divisions:'Divisiones', members:'Integrantes' }
    : {};
  const preferred = type === 'user' ? ['name','email','role','role_code','whatsapp','dni','status','birth_date','student_course_name','student_division_name','course','division','title'] : (type === 'team' ? ['id','name','project','description','teachers','courses','divisions','members','student_names','logo_url'] : Object.keys(row));
