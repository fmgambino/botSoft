...
function renderNotifications() {
  ...
  return `<section class="card glass"><div class="section-header"><div><h3>Centro de notificaciones</h3><p class="muted">${helper}</p></div><div class="toolbar">${button}</div></div>
    <div class="toolbar filters-row"><label class="inline-check"><input type="checkbox" id="notificationsSelectAll"> Seleccionar todo</label><button class="btn btn-secondary btn-sm" id="markSelectedReadBtn">Marcar leídas</button><button class="btn btn-secondary btn-sm" id="markSelectedUnreadBtn">Marcar sin leer</button><button class="btn btn-secondary btn-sm" id="clearUnreadBtn">Limpiar contador</button></div>
    <div class="list-simple">${rows.map(n => `<div class="notification-row list-item"><input type="checkbox" class="notification-row-check" data-id="${escapeAttr(n.id)}"><button class="notification-select notification-inner" data-id="${escapeAttr(n.id)}"><div><strong>${escapeHtml(n.title)}</strong><span>${notificationMessage(n)}</span></div>${n.unread ? '<span class="status-pill status-pending">Sin leer</span>' : '<span class="status-pill status-approved">Leída</span>'}</button></div>`).join('') || '<p class="muted">No hay notificaciones.</p>'}</div></section>`;
}
...
async function bulkMarkNotifications(read=true, allUnread=false) {
  ...
  try {
    ...
    state.notifications.forEach(n => { if (ids.map(String).includes(String(n.id))) n.unread = !read; });
    await refreshSupabaseData(); renderView(); updateHeader(); showToast('Notificaciones actualizadas');
    ...
    reader.onload = async () => {
      try {
        if (module === 'inventory') {
          ...
          if (window.sb?.enabled) { state.inventory = dedupeInventoryRows(await window.sb.listInventory()); }
          state.inventoryPage = 1; renderView();
