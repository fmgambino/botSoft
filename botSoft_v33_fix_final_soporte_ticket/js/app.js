...
function renderNotifications() {
  ...
  return `<section class="card glass notifications-panel">
    ...
    <div class="notifications-list">${rows.map(n => `<article class="notification-card ${n.unread ? 'is-unread' : 'is-read'}"${notificationAccentStyle(n)}>
...
function renderNotificationDropdown() {
  notificationsPreview.innerHTML = state.notifications.length ? state.notifications.slice(0, 4).map(n => `
    <button class="notification-row notification-select ${n.unread ? 'unread' : ''}" data-id="${escapeAttr(n.id)}"${notificationAccentStyle(n)}>
...
async function openTicketForm(ticket=null) {
  ...
  try {
    ...
    state.notifications.unshift({ id:`local-${Date.now()}`, title:'Nuevo ticket de soporte', message:msg, unread:true, section:'supportTickets', created_at:new Date().toISOString(), sender_name:saved.requester_name || state.user.name || 'Sistema' });
