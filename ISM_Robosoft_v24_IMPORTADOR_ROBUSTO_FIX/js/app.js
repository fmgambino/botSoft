...
async function openUserProfileForm() {
  const result = await Swal.fire({
    ...
    html: `<div class="swal-form-grid">
      ...
      <label><span>Rol</span><input class="swal2-input" value="Alumno" disabled><input id="userRole" type="hidden" value="Alumno"></label>
      ...
      <label><span>Curso</span><select id="userCourse" class="swal2-select"><option value="">Elegir de 1 a 7º</option><option>1º</option><option>2º</option><option>3º</option><option>4º</option><option>5º</option><option>6º</option><option>7º</option></select></label><label><span>División</span><select id="userDivision" class="swal2-select"><option value="">Elegir de A al C</option><option>A</option><option>B</option><option>C</option></select></label>
    ...
    didOpen: () => {
      ...
      // Rol fijo Alumno; curso y división visibles.
    },
    preConfirm: async () => {
      ...
      return { name, full_name: name, email, password: document.getElementById('userPassword').value.trim(), role: 'Alumno', dni: document.getElementById('userDni').value.trim(), whatsapp: document.getElementById('userWhatsapp').value.trim(), birth_date: document.getElementById('userBirthDate').value || null, title: null, student_course_name: document.getElementById('userCourse').value || null, student_division_name: document.getElementById('userDivision').value || null, avatar_url };
    }
  });
  if (!result.isConfirmed) return;
  try {
