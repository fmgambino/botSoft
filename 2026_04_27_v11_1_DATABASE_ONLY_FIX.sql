-- ISM Robosoft v11.1 - DATABASE ONLY FIX
-- Corrige:
-- 1) admin_update_inventory_asset con defaults/firma duplicada
-- 2) create_loan_request faltante para Gestión de préstamos
-- 3) loans_frontend_view sin error de cambio de columnas
-- 4) notificaciones básicas conectadas a préstamos e inventario
-- Ejecutar completo en Supabase SQL Editor.

begin;

-- =========================================================
-- 0) LIMPIEZA SEGURA DE FUNCIONES CON FIRMAS CONFLICTIVAS
-- =========================================================
-- Este DROP exacto corrige:
-- ERROR 42P13: cannot remove parameter defaults from existing function
-- HINT: Use DROP FUNCTION admin_update_inventory_asset(uuid,text,uuid,uuid,text,uuid,text,text,text,uuid,uuid,text) first.

drop function if exists public.admin_update_inventory_asset(uuid,text,uuid,uuid,text,uuid,text,text,text,uuid,uuid,text) cascade;
drop function if exists public.admin_update_inventory_asset(uuid,text,text,text,text,text,text,text,text,text,text,text) cascade;
drop function if exists public.admin_update_inventory_asset(uuid,text,uuid,uuid,text,uuid,text,text,uuid,uuid,uuid,text) cascade;

drop function if exists public.create_loan_request(text,date,time,time,text[],text) cascade;
drop function if exists public.create_loan_request(text,text,text,text,text[],text) cascade;
drop function if exists public.create_loan_request(text,text,text,text,jsonb,text) cascade;

drop function if exists public.admin_update_loan_status(uuid,text) cascade;

-- =========================================================
-- 1) RPC ÚNICA PARA EDITAR INVENTARIO
-- =========================================================
...
begin
  ...
  if nullif(trim(coalesce(p_location_code,'')), '') is not null then
    ...
    where code = trim(p_location_code) or name = trim(p_location_code)
    limit 1;
  ...
  if nullif(trim(coalesce(p_status,'')), '') is not null then
    ...
    from public.inventory_status_catalog
    where code = lower(regexp_replace(trim(p_status), '\s+', '_', 'g'))
       or name = trim(p_status)
       or frontend_status = trim(p_status)
    limit 1;
  end if;
  ...
  set barcode = coalesce(nullif(trim(coalesce(p_barcode,'')), ''), barcode),
      serial_number = coalesce(nullif(trim(coalesce(p_serial_number,'')), ''), serial_number),
      ...
      location_detail = coalesce(nullif(trim(coalesce(p_location_detail,'')), ''), location_detail),
      zone = coalesce(nullif(trim(coalesce(p_zone,'')), ''), zone),
      updated_at = now()
  where id = p_asset_id;
...
-- =========================================================
-- 3) COLUMNAS NECESARIAS PARA PRÉSTAMOS
-- =========================================================
...
-- Recreamos la vista con DROP, no CREATE OR REPLACE, para evitar:
-- cannot change name of view column "requester" to "requester_id"
...
select
  ...
  coalesce(l.use_date::text, l.requested_at::date::text) as use_date,
...
-- =========================================================
-- 4) RPC PARA CREAR SOLICITUD DE PRÉSTAMO
-- =========================================================
-- Firma pensada para Supabase RPC con named params:
-- p_from_time, p_items, p_notes, p_team, p_to_time, p_use_date
create function public.create_loan_request(
  p_team text default null,
  p_use_date text default null,
...
begin
  ...
  v_use_date := nullif(p_use_date, '')::date;
  v_from_time := nullif(p_from_time, '')::time;
  v_to_time := nullif(p_to_time, '')::time;

  select id into v_team_id
  from public.teams
  where name = p_team or id::text = p_team
  limit 1;
  ...
  foreach v_item in array coalesce(p_items, array[]::text[]) loop
    ...
    from public.inventory_assets
    where asset_code = v_item or barcode = v_item or id::text = v_item
    limit 1;
  ...
  -- Notificar a administradores activos.
  insert into public.notifications(recipient_profile_id, title, message, section, created_by)
  select p.id,
         'Nueva solicitud de préstamo',
         coalesce(v_requester_name,'Un usuario') || ' registró una solicitud de préstamo.',
         'loanManagement',
         auth.uid()
  from public.profiles p
  join public.roles r on r.id = p.role_id
...
grant execute on function public.create_loan_request(text,text,text,text,text[],text) to authenticated;

-- =========================================================
-- 5) RPC PARA CAMBIAR ESTADO DE PRÉSTAMO
-- =========================================================
create function public.admin_update_loan_status(
  p_loan_id uuid,
  p_status_label text
)
returns void
...
begin
  v_status := case p_status_label
    ...
    else coalesce(lower(p_status_label), 'abierto')
  end;

  update public.inventory_loans
  set status = v_status,
      updated_at = now(),
      approved_by = case when v_status in ('aprobado','rechazado') then auth.uid() else approved_by end,
      approved_at = case when v_status in ('aprobado','rechazado') then now() else approved_at end
  where id = p_loan_id
  ...
  if v_requester is not null then
    ...
    values (
      v_requester,
      'Solicitud de préstamo actualizada',
      'Tu solicitud fue marcada como ' || coalesce(p_status_label, v_status) || '.',
      'loanManagement',
      auth.uid()
    );
  end if;
...
-- =========================================================
-- 6) NOTIFICACIÓN AUTOMÁTICA AL INGRESAR INVENTARIO
-- =========================================================
...
begin
  ...
  select p.id,
         ...
         'Se registró: ' || coalesce(v_name, new.asset_code, new.barcode),
         'inventory',
         auth.uid()
  from public.profiles p
  where p.is_active is true;
...
create trigger trg_notify_inventory_asset_insert
after insert on public.inventory_assets
...
-- Docentes/alumnos sin Usuarios ni ABM de Inventario.
...
-- Préstamos y notificaciones para todos.
