...
function code128CModules(value) {
  // Code-128 Set C: estándar, numérico por pares. Si llega impar, se antepone 0.
  ...
  return { digits, patterns: values.map(v => patterns[v]) };
}
function barcodePatternSvg(value, opts = {}) {
  const { digits, patterns } = code128CModules(value);
  const moduleW = Number(opts.moduleW || 2);
  const height = Number(opts.height || 64);
  let x = 10;
  const bars = [];
  for (const pat of patterns) {
    for (let i = 0; i < pat.length; i++) {
      const w = Number(pat[i]) * moduleW;
      if (i % 2 === 0) bars.push(`<rect x="${x}" y="8" width="${w}" height="${height-18}" fill="#000"/>`);
      x += w;
    }
  }
  const vb = x + 10;
  return `<svg class="barcode-svg code128-svg" viewBox="0 0 ${vb} ${height}" preserveAspectRatio="none" role="img" aria-label="Code 128 ${escapeAttr(digits)}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" rx="8" fill="#fff"/>${bars.join('')}</svg>`;
}
function barcodePreviewMarkup(value, small = false) {
  const numeric = String(value || '8435439000001').replace(/\D/g, '') || '8435439000001';
  return `<div class="barcode-visual ${small ? 'barcode-visual-sm' : ''}">${barcodePatternSvg(numeric)}<strong>${escapeHtml(numeric)}</strong></div>`;
}
...
function renderNotifications() {
  ...
  const button = isCurrentStudent() ? '' : '<button class="btn btn-primary btn-sm" id="massNotifyBtn">Enviar notificación</button>';
...
function viewRecord(type, row) {
  ...
  const html = `<div class="user-detail-card">${type === 'user' ? `<div class="user-detail-header"><img src="${escapeHtml(avatarSrc)}" onerror="this.src='./assets/avatar-default.svg'"/><div><h3>${escapeHtml(row.name || '-')}</h3><p>${escapeHtml(row.email || '-')}</p><span class="status ${row.status === 'Activo' ? 'ok' : 'warn'}">${escapeHtml(row.status || '-')}</span></div></div>` : ''}${invImage}<div class="detail-grid pretty">${preferred.filter(k => k in row).map(k => `<div class="${k === 'title' || k === 'image_url' ? 'full-span' : ''}"><strong>${escapeHtml(labels[k] || k)}</strong><span>${k === 'image_url' && row[k] ? `<a href="${escapeAttr(row[k])}" target="_blank">Ver imagen</a>` : escapeHtml(k === 'title' ? certText : (Array.isArray(row[k]) ? row[k].join(', ') : row[k] || '-'))}</span></div>`).join('')}</div></div>`;
...
async function editUserRecord(row) {
  ...
  if (!result.isConfirmed) return;
  try { if (window.sb?.enabled) await window.sb.updateProfileFull(row.id, result.value); Object.assign(row, result.value); await refreshSupabaseData(); renderView(); showToast('Usuario actualizado'); }
...
async function refreshSupabaseData() {
  ...
  try {
    const [profile, users, inventory, teams, loans, roles, notifications, conditions, permissions] = await Promise.all([
...
function setupInventoryRealtime() {
  if (!window.sb?.enabled || !window.sb.client || state.__inventoryRealtimeReady) return;
  state.__inventoryRealtimeReady = true;
  try {
    window.sb.client.channel('inventory-assets-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'inventory_assets' }, async () => {
        try { state.inventory = dedupeInventoryRows(await window.sb.listInventory()); if (state.currentView === 'inventory') renderView(); updateHeader(); } catch(e) { console.warn(e); }
      })
      .subscribe();
  } catch (e) { console.warn('Realtime inventario no disponible', e); }
}
