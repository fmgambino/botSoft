...
const DEFAULT_PERMISSIONS = [
  'dashboard.read', 'users.read', 'users.manage', 'roles.manage', 'teams.read', 'teams.manage',
  'inventory.read', 'inventory.manage', 'inventory.loan', 'inventory.approve',
  'loan.read', 'loan.create', 'loan.manage', 'notifications.read', 'notifications.send',
...
async function duplicateInventoryAsset(row) {
  ...
  const copy = { ...row, id: null, code: '', barcode: '', serial: row.serial || '', item: `${row.item || 'Insumo'} (copia)` };
    ...
    didOpen: () => {
      ...
      if (btn) btn.addEventListener('click', async () => {
        ...
        if (r.isConfirmed && r.value.name) {
          ...
          await Swal.fire({ icon:'success', title:'Condición guardada', text:'La condición fue agregada en Supabase con su color.' });
        }
      });
      ...
      if (editBtn) editBtn.addEventListener('click', async () => {
        ...
        if (r.isConfirmed) {
          ...
          await Swal.fire({ icon:'success', title:'Color actualizado', text:'El color de la condición fue guardado en Supabase.' });
        }
      });
    },
  ...
  try {
    if (window.sb?.enabled) {
      if (result.value.condition) {
        ...
        await window.sb.saveInventoryCondition(result.value.condition, cond?.color || '#64748b');
      }
