...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async saveInventoryCondition(name, color = '#64748b', options = {}) {
      ...
      const rpc = await client.rpc('inventory_condition_save_v19', {
        p_name: cleanName,
        p_color: cleanColor,
        p_fail_on_duplicate: Boolean(options.failOnDuplicate)
      });
      if (!rpc.error) return rpc.data;
      if (String(rpc.error.message || '').toLowerCase().includes('duplicada')) throw rpc.error;

      const { data: list, error: listError } = await client.from('inventory_conditions').select('id, name, color');
      if (listError) throw listError;
      const existing = (list || []).find(c => String(c.name || '').trim().toLowerCase() === cleanName.toLowerCase());
      if (existing && options.failOnDuplicate) throw new Error('Condición duplicada: ya existe una condición con ese nombre.');
      if (existing?.id) {
        const { data, error } = await client
          .from('inventory_conditions')
    ...
    async bulkUpdateInventoryAssets(ids, changes) {
      const { error } = await assertClient().rpc('bulk_update_inventory_assets_v19', {
        p_asset_ids: ids || [],
