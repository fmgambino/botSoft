...
function renderNotifications() {
  ...
  return `<section class="card glass notifications-panel">
    ...
    <div class="notifications-toolbar">
      <label class="inline-check select-all"><input type="checkbox" id="notificationsSelectAll"> <span>Seleccionar todo</span></label>
    ...
    <div class="notifications-list">${rows.map(n => `<article class="notification-card ${n.unread ? 'is-unread' : 'is-read'}">
...
async function refreshSupabaseData() {
  ...
  try {
    const [profile, users, inventory, teams, loans, roles, notifications, conditions, permissions, supportTickets, ticketStatuses, ticketIncidents] = await Promise.all([
...
if (!nav.some(n => n.key === 'supportTickets')) nav.splice(nav.findIndex(n => n.key === 'accessControl'), 0, { key:'supportTickets', icon:'ticket', label:{ administrator:'Soporte Ticket', teacher:'Soporte Ticket' } });
...
function ticketPager(rows, position='top') {
  ...
  return `<div class="toolbar pager-row ${position}"><label>Listar <select data-ticket-page-size>${sizes.map(s=>`<option value="${s}" ${state.ticketPageSize===s?'selected':''}>${s}</option>`).join('')}</select></label><button class="btn btn-secondary btn-sm" data-ticket-page="prev" ${state.ticketPage<=1?'disabled':''}>Anterior</button><span class="muted">Página ${state.ticketPage} de ${totalPages} · ${rows.length} registros</span><button class="btn btn-secondary btn-sm" data-ticket-page="next" ${state.ticketPage>=totalPages?'disabled':''}>Siguiente</button></div>`;
}
...
async function openTicketForm(ticket=null) {
  ...
  const res = await Swal.fire({ title: ticket?'Editar ticket':'Nuevo ticket', width:900, html:`<div class="swal-form-grid"><label><span>N° ticket</span><input id="ticketNumber" class="swal2-input" value="${escapeHtml(ticket?.ticket_number || ticketNumber())}" readonly></label><label><span>Sala / ubicación</span><select id="ticketRoom" class="swal2-select">${roomOptions(ticket?.room||'')}</select></label><label><span>Equipo o insumo del inventario</span><select id="ticketAsset" class="swal2-select">${invOptions}</select></label><label><span>Tipo de incidencia</span><select id="ticketIncident" class="swal2-select">${incidentOptions}</select></label><label><span>Estado</span><select id="ticketStatus" class="swal2-select" ${canManageTickets()?'':'disabled'}>${statusOptions}</select></label><label><span>Prioridad</span><select id="ticketPriority" class="swal2-select"><option ${ticket?.priority==='Normal'?'selected':''}>Normal</option><option ${ticket?.priority==='Alta'?'selected':''}>Alta</option><option ${ticket?.priority==='Urgente'?'selected':''}>Urgente</option><option ${ticket?.priority==='Baja'?'selected':''}>Baja</option></select></label><label class="full-span"><span>Título</span><input id="ticketTitle" class="swal2-input" value="${escapeHtml(ticket?.title || '')}"></label><label class="full-span"><span>Descripción</span><textarea id="ticketDescription" class="swal2-textarea">${escapeHtml(ticket?.description || '')}</textarea></label><label class="full-span"><span>Historial / resolución</span><textarea id="ticketHistory" class="swal2-textarea">${escapeHtml(ticket?.history || '')}</textarea></label></div>`, showCancelButton:true, confirmButtonText:'Guardar', cancelButtonText:'Cancelar', preConfirm:()=>{ const title=document.getElementById('ticketTitle').value.trim(); if(!title) return Swal.showValidationMessage('Ingresá un título'); const assetId=document.getElementById('ticketAsset').value; const asset=(state.inventory||[]).find(i=>String(i.id)===String(assetId)); return { id: ticket?.id || crypto.randomUUID?.() || Date.now(), ticket_number:document.getElementById('ticketNumber').value, title, description:document.getElementById('ticketDescription').value.trim(), room:document.getElementById('ticketRoom').value, asset_id:assetId||null, asset_label:asset?`${asset.code} · ${asset.item}`:'', incident_type:document.getElementById('ticketIncident').value, status:canManageTickets()?document.getElementById('ticketStatus').value:(ticket?.status||'pendiente'), priority:document.getElementById('ticketPriority').value, history:document.getElementById('ticketHistory').value.trim(), requester_id:state.user.id||null, requester_name:state.user.name||state.user.full_name||'Usuario', created_at:ticket?.created_at||new Date().toISOString(), updated_at:new Date().toISOString() }; }});
  if (!res.isConfirmed) return;
  try { if (window.sb?.enabled && window.sb.saveSupportTicket) await window.sb.saveSupportTicket(res.value); const idx=(state.supportTickets||[]).findIndex(t=>String(t.id)===String(res.value.id)); if(idx>=0) state.supportTickets[idx]=res.value; else state.supportTickets.unshift(res.value); await notifyTelegram(`Ticket ${res.value.ticket_number}: ${res.value.title} (${ticketStatusMeta(res.value.status).name})`); await refreshSupabaseData(); renderView(); showToast('Soporte Ticket','Ticket guardado'); } catch(e) { Swal.fire({icon:'error', title:'No se pudo guardar', text:e.message}); }
}
async function manageTicketIncidents() {
  ...
  if (res.isConfirmed && res.value.name) { if (window.sb?.enabled && window.sb.saveTicketIncident) await window.sb.saveTicketIncident(res.value); state.ticketIncidents.push(res.value); renderView(); }
}
async function notifyTelegram(message) {
  const cfg = state.telegramConfig || {};
  if (!cfg.enabled || !cfg.botToken || !cfg.chatId) return;
  try { await fetch(`https://api.telegram.org/bot${encodeURIComponent(cfg.botToken)}/sendMessage`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ chat_id:cfg.chatId, text:message }) }); } catch(e) { console.warn('Telegram no disponible', e); }
}
async function bulkTickets(action) {
  const ids = Array.from(state.selectedTicketIds);
  if (!ids.length) return;
  if (action === 'delete') { const ok = await Swal.fire({icon:'warning', title:'Eliminar tickets', text:`Se eliminarán ${ids.length} tickets.`, showCancelButton:true, confirmButtonText:'Eliminar'}); if(!ok.isConfirmed) return; if(window.sb?.enabled && window.sb.deleteSupportTickets) await window.sb.deleteSupportTickets(ids); state.supportTickets = state.supportTickets.filter(t=>!ids.includes(String(t.id))); state.selectedTicketIds.clear(); renderView(); return; }
  ...
  if(res.isConfirmed){ const status=document.getElementById('bulkTicketStatus').value; if(window.sb?.enabled && window.sb.bulkUpdateSupportTickets) await window.sb.bulkUpdateSupportTickets(ids,{status}); state.supportTickets.forEach(t=>{if(ids.includes(String(t.id))) t.status=status;}); renderView(); }
}
