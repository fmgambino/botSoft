...
async function openTicketForm(ticket=null) {
  ...
  try { if (window.sb?.enabled && window.sb.saveSupportTicket) await window.sb.saveSupportTicket(res.value); const idx=(state.supportTickets||[]).findIndex(t=>String(t.id)===String(res.value.id)); if(idx>=0) state.supportTickets[idx]=res.value; else state.supportTickets.unshift(res.value); await notifyTelegram(`Ticket ${res.value.ticket_number}: ${res.value.title} (${ticketStatusMeta(res.value.status).name})`); await refreshSupabaseData(); renderView(); await Swal.fire({icon:'success', title:'Ticket guardado', timer:1400, showConfirmButton:false}); } catch(e) { Swal.fire({icon:'error', title:'No se pudo guardar', text:e.message}); }
}
async function manageTicketIncidents() {
  ...
  if (res.isConfirmed && res.value.name) { if (window.sb?.enabled && window.sb.saveTicketIncident) await window.sb.saveTicketIncident(res.value); state.ticketIncidents.push(res.value); renderView(); await Swal.fire({icon:'success', title:'Incidencia agregada', timer:1400, showConfirmButton:false}); }
}
