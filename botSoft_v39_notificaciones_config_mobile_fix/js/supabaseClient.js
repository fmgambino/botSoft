...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async listNotifications() {
      ...
      const rpc = await c.rpc('list_my_notifications_v39');
      if (!rpc.error) {
        return (rpc.data || []).map(n => ({ id:n.id, title:n.title, message:n.message, sender:n.sender_name || 'Sistema', sender_name:n.sender_name || 'Sistema', unread: !n.read_at, section:n.section || 'notifications', created_at:n.created_at, read_at:n.read_at || null, recipient_profile_id:n.recipient_profile_id || '', recipient_name:n.recipient_name || '', recipient_role:n.recipient_role || '' }));
      }
      const { data: userData } = await c.auth.getUser();
      const uid = userData?.user?.id || '';
      let q = c.from('notifications').select('id,title,message,section,created_by,created_at,read_at,recipient_profile_id').order('created_at', { ascending: false }).limit(2000);
    ...
    async markNotifications(ids, read = true) {
      ...
      const uuidList = list;
      const rpc39 = await assertClient().rpc('set_notifications_read_state_v39', { p_notification_ids: uuidList, p_read: read });
      if (!rpc39.error) return rpc39.data;
      const rpc28 = await assertClient().rpc('set_notifications_read_state_v28', { p_notification_ids: uuidList, p_read: read });
    ...
    async logUserLogin() {
      const rpc39 = await assertClient().rpc('notify_user_login_v39');
      if (!rpc39.error) return rpc39.data;
      const rpc = await assertClient().rpc('notify_user_login_v37');
      if (rpc.error) console.warn('notify_user_login', rpc.error);
      return rpc.data;
    },
    async logUserLogout() {
      const rpc39 = await assertClient().rpc('notify_user_logout_v39');
      if (!rpc39.error) return rpc39.data;
      const rpc = await assertClient().rpc('notify_user_logout_v37');
      if (rpc.error) console.warn('notify_user_logout', rpc.error);
      return rpc.data;
    },
