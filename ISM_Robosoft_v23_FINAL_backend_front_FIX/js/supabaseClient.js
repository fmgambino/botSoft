...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async listTeams() {
      const { data, error } = await assertClient().rpc('admin_list_teams_full_v23');
      if (error) throw error;
      return (data || []).map(t => ({
    ...
    async updateInventoryAsset(id, payload) {
      ...
      const { error } = await assertClient().rpc('admin_update_inventory_asset_v23', {
        p_asset_id: assetId,
        p_name: payload.item,
    ...
    async createInventoryAsset(payload) {
      const { data, error } = await assertClient().rpc('admin_create_inventory_asset_v23', {
        p_name: payload.item,
        p_category: payload.category || payload.type || 'General',
        p_asset_code: payload.code || null,
        p_serial_number: payload.serial || null,
