  ...
  return data;
    ...
    async listNotifications() {
      const { data, error } = await assertClient()
        ...
        .order('created_at', { ascending: false });
      if (error) throw error;
      return (data || []).map(n => ({ id:n.id, title:n.title, message:n.message, sender:n.sender || n.sender_name || n.created_by_name || 'Sistema', sender_name:n.sender_name || n.sender || 'Sistema', unread: !n.read_at, section:n.section || 'notifications', created_at:n.created_at, read_at:n.read_at || null, recipient_name:n.recipient_name || n.recipient_full_name || '', recipient_role:n.recipient_role || n.recipient_role_name || '' }));
    },
    ...
    async saveSupportTicket(payload) {
      ...
      const rpc = await assertClient().rpc('support_ticket_save_v34', { p_ticket: row });
      if (!rpc.error) return rpc.data;
      const { data, error } = await assertClient().from('support_tickets').upsert(row).select('*').single();
      if (error) throw error;
      return data;
    },
    async deleteSupportTickets(ids) {
      const list = (ids || []).map(String).filter(Boolean);
      if (!list.length) return;
      const rpc = await assertClient().rpc('support_ticket_delete_v34', { p_ids: list });
      if (!rpc.error) return;
    ...
    async bulkUpdateSupportTickets(ids, changes) {
      const list = (ids || []).map(String).filter(Boolean);
      if (!list.length) return;
      const rpc = await assertClient().rpc('support_ticket_bulk_update_v34', { p_ids: list, p_changes: { ...changes, updated_at: new Date().toISOString() } });
      if (!rpc.error) return;
