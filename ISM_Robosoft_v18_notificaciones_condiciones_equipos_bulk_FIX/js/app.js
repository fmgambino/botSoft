...
async function openBulkInventoryEdit() {
  ...
  const result = await Swal.fire({
    ...
    preConfirm: () => ({ status: document.getElementById('bulkStatus').value || null, condition: document.getElementById('bulkCondition').value || null })
  });
  if (!result.isConfirmed) return;
  if (!result.value.status && !result.value.condition) return Swal.fire({ icon:'info', title:'Sin cambios', text:'Elegí estado o condición.' });
  try {
    if (window.sb?.enabled) await window.sb.bulkUpdateInventoryAssets(ids, result.value);
    state.inventory.forEach(i => { if (ids.includes(String(i.id))) { if (result.value.status) i.status = result.value.status; if (result.value.condition) i.condition = result.value.condition; } });
    ...
    didOpen: () => {
      ...
      if (btn) btn.addEventListener('click', async () => {
        ...
        if (r.isConfirmed && r.value.name) {
          ...
          if (window.sb?.enabled) {
            try {
              await window.sb.saveInventoryCondition(cleanName, cleanColor);
...
async function openRoleForm(role=null) {
  const permissionOptions = (state.permissionsData || []).map(p => p.code).filter(Boolean);
  if (!permissionOptions.length) permissionOptions.push('dashboard.read','users.read','users.manage','roles.manage','teams.read','teams.manage','inventory.read','inventory.manage','inventory.loan','inventory.approve','loan.read','loan.create','loan.manage','notifications.read','notifications.send','courses.read','courses.manage','library.read','library.manage');
