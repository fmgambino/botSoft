...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async listInventoryConditions() {
      ...
      (data || []).forEach(c => {
        const cleanName = String(c.name || '').trim().replace(/\s+/g, ' ');
        if (!cleanName) return;
        const cleanColor = /^#[0-9a-f]{6}$/i.test(String(c.color || '').trim()) ? String(c.color).trim() : '#64748b';
        byName.set(cleanName.toLowerCase(), { id: c.id, name: cleanName, color: cleanColor, sort_order: c.sort_order || 100 });
      });
    ...
    async saveInventoryCondition(name, color = '#64748b') {
      ...
      // RPC v16 nueva para evitar conflictos con funciones antiguas cacheadas.
      const rpc = await client.rpc('save_inventory_condition_v16', { p_name: cleanName, p_color: cleanColor });
      if (!rpc.error) return rpc.data;

      // Compatibilidad con instalaciones que todavía no ejecutaron el SQL v16.
      const rpcLegacy = await client.rpc('upsert_inventory_condition', { p_name: cleanName, p_color: cleanColor });
      if (!rpcLegacy.error) return rpcLegacy.data;

      // Fallback directo: busca por nombre normalizado y actualiza el color real en la tabla.
      const { data: list, error: listError } = await client
        .from('inventory_conditions')
        .select('id, name')
        .eq('is_active', true);
      if (listError) throw listError;
      const existing = (list || []).find(c => String(c.name || '').trim().toLowerCase() === cleanName.toLowerCase());
      if (existing?.id) {
        const { data, error } = await client
          .from('inventory_conditions')
          .update({ name: cleanName, color: cleanColor, is_active: true, updated_at: new Date().toISOString() })
          .eq('id', existing.id)
      ...
      const { data, error } = await client
        .from('inventory_conditions')
        .insert({ name: cleanName, color: cleanColor, is_active: true, updated_at: new Date().toISOString() })
        .select('id, name, color')
        .single();
      if (error) throw error;
      return data;
    ...
    async listTeams() {
      const { data, error } = await assertClient().rpc('admin_list_teams_full');
      if (error) throw error;
      return (data || []).map(t => ({ id:t.id, name:t.name, project:t.project || '', description:t.description || '', mentor_id:t.mentor_id || null, mentor_backup_id:t.mentor_backup_id || null, student_ids:Array.isArray(t.student_ids)?t.student_ids.map(String):[], teachers:Array.isArray(t.teachers)?t.teachers:[], students:Array.isArray(t.student_ids)?t.student_ids.length:(t.students||0), courses:Array.isArray(t.courses)?t.courses:[], divisions:Array.isArray(t.divisions)?t.divisions:[] }));
    },
    async listLoans() {
    ...
    async bulkUpdateInventoryAssets(ids, changes) {
      const { error } = await assertClient().rpc('bulk_update_inventory_assets', { p_asset_ids: ids || [], p_status: changes?.status || null, p_condition_note: changes?.condition || null });
      if (error) throw error;
    },
