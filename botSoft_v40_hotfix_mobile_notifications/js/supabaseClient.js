...
async sendPasswordReset(email) {
  ...
  return data;
    ...
    async logUserLogin() {
      ...
      const rows = targets.map(p => ({ recipient_profile_id:p.id, title:'Inicio de sesión', message:`Usuario: ${actor} · Rol: ${role} · Fecha: ${new Date().toLocaleString('es-AR', { timeZone:'America/Argentina/Buenos_Aires' })}`, section:'access', created_by:uid, created_at:now }));
      if (rows.length) await c.from('notifications').insert(rows).then(()=>{},()=>{});
      return true;
    },
    async logUserLogout() {
      const c = assertClient();
