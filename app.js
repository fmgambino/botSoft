...
function renderTeacherCampus() {
  ...
  return `<section class="card glass"><div class="section-header"><div><h3>Gestión de cursos</h3><p class="muted">Creación de cursos, visibilidad, códigos de acceso y contenido tipo e-learning.</p></div><div class="toolbar"><button class="btn btn-secondary btn-sm" data-import="courses">${icons.import} Importar CSV</button><button class="btn btn-secondary btn-sm" data-export="courses">${icons.export} Exportar CSV</button><button class="btn btn-primary btn-sm" data-create="module">Nuevo curso</button></div></div><div class="grid-cards">${rows.map(mod => `<article class="card glass-soft"><div class="metric-inline"><h3>${mod.title}</h3>${statusPill(mod.status)}</div><p><strong>Docente:</strong> ${mod.teacher}</p><p><strong>Visibilidad:</strong> ${mod.visibility}</p><p><strong>Código:</strong> ${mod.code}</p><p><strong>Clave:</strong> ${mod.accessKey}</p><p><strong>Equipo/alcance:</strong> ${mod.team}</p><div class="course-metrics"><span>${mod.lessons} lecciones</span><span>${mod.tasks} tareas</span><span>${mod.evaluations} evaluativos</span></div></article>`).join('')}</div></section>`;
}
  ...
  return `
    <section class="layout-two">
      <article class="card glass">
        ...
        <div class="grid-cards">
          ${enrolled.map(course => `<article class="card glass-soft"><div class="metric-inline"><h3>${course.title}</h3>${statusPill('Inscripto')}</div><p><strong>Docente:</strong> ${course.teacher}</p><p><strong>Equipo:</strong> ${course.team}</p><div class="course-metrics"><span>${course.lessons} módulos/lecciones</span><span>${course.tasks} tareas</span><span>${course.evaluations} evaluativos</span></div><p><strong>Progreso:</strong> ${course.progress}%</p></article>`).join('')}
        </div>
      </article>
      <aside class="card glass">
...
function renderLibrary() {
  return `<section class="card glass"><div class="section-header"><div><h3>Biblioteca digital</h3><p class="muted">Repositorio de documentos, videos y material del laboratorio.</p></div><div class="toolbar"><button class="btn btn-secondary btn-sm" data-import="library">${icons.import} Importar CSV</button><button class="btn btn-secondary btn-sm" data-export="library">${icons.export} Exportar CSV</button></div></div><div class="file-grid">${state.library.map(file => `<article class="file-item glass-soft"><strong>${file.title}</strong><p>${file.area}</p><span class="tag">${file.type}</span></article>`).join('')}</div></section>`;
}
...
function renderView() {
  updateHeader();
  renderNav();
  const views = { dashboard: renderDashboard, users: renderUsers, roles: renderRoles, team: renderTeams, inventory: renderInventory, courses: renderCourses, library: renderLibrary, notifications: renderNotifications, profile: renderProfile, settings: renderSettings };
...
async function openLoanRequest() {
  ...
  const result = await Swal.fire({
    ...
    html: `
      <div class="swal-form-grid">
        <label><span>Curso / equipo</span><input id="loanCourse" class="swal2-input" placeholder="Ej. Equipo Rover A / 6°C"></label>
        <label><span>Fecha de uso</span><input id="loanDate" type="date" class="swal2-input"></label>
        <label class="full-span"><span>Insumos (1 o más)</span><select id="loanItems" class="swal2-select" multiple size="6">${items}</select></label>
        <label class="full-span"><span>Observaciones</span><textarea id="loanObs" class="swal2-textarea" placeholder="Detalle del pedido y condición esperada"></textarea></label>
      </div>`,
    preConfirm: () => {
      ...
      if (!course || !date || !selected.length) {
        Swal.showValidationMessage('Completá curso/equipo, fecha y al menos un insumo.');
        return false;
      }
      return { course, date, selected };
    }
  });
  if (!result.isConfirmed) return;
