...
async function importInventoryCsv(file, text) {
  ...
  const mapped = rows.map((row, idx) => {
    ...
    const supplier = clean(csvField(row, ['proveedor', 'empresa'])); const location = clean(csvField(row, ['ubicacion', 'ubicación', 'domicilio', 'locacion'])) || 'Laboratorio de Robótica';
...
async function saveGeneralSettings() {
  try {
    ...
    const payload = {
      institutionName: document.getElementById('institutionNameInput')?.value?.trim() || 'Instituto San Miguel',
...
function roomOptions(selected='') { return ['Laboratorio de Robótica','Sala N°1 - Planta Baja (PB)','Sala N°2 - Primer Piso'].map(x => `<option ${selected===x?'selected':''}>${x}</option>`).join(''); }
...
function defaultBrandSettings(){ return { institutionName:'Instituto San Miguel', institutionSubtitle:'Laboratorio de Robótica', institutionEmail:'robotica@ism.edu.ar', logoDataUrl:'', pollingSeconds:25 }; }
...
function applyBranding(){
  ...
  document.querySelectorAll('.brand-copy strong').forEach(el => el.textContent = gs.institutionName || 'Instituto San Miguel');
  document.querySelectorAll('.brand-copy span').forEach(el => el.textContent = gs.institutionSubtitle || 'Laboratorio de Robótica');
}
...
async function saveGeneralSettings(silent=false) {
  try {
    ...
    const payload = {
      institutionName: document.getElementById('institutionNameInput')?.value?.trim() || 'Instituto San Miguel',
      institutionSubtitle: document.getElementById('institutionSubtitleInput')?.value?.trim() || 'Laboratorio de Robótica',
      institutionEmail: document.getElementById('institutionEmailInput')?.value?.trim() || 'robotica@ism.edu.ar',
      logoDataUrl: logoDataUrl || '',
...
(function(){
  ...
  renderSettings = function() {
    const gs = getBrandSettings ? getBrandSettings() : {institutionName:'Instituto San Miguel', institutionSubtitle:'Laboratorio de Robótica', institutionEmail:'', logoDataUrl:'', pollingSeconds:25};
    const cfg = state.telegramConfig || {};
    const logoPreview = `<div class="settings-logo-preview"><img id="settingsLogoPreviewImg" src="${escapeAttr(gs.logoDataUrl || './assets/img/logo-Coordinacion-Robotica.png')}" alt="Logo institucional"></div>`;
...
/* === V46: inventory notification dedupe + robust notification navigation/toasts === */
(function(){
  function routeNotificationV46(n){
    const hay = `${n?.section||''} ${n?.module||''} ${n?.target_view||''} ${n?.title||''} ${n?.message||''}`.toLowerCase();
    const explicit = String(n?.section || n?.module || n?.target_view || '').trim();
    const valid = new Set(['dashboard','users','roles','team','inventory','loanManagement','supportTickets','accessControl','courses','library','notifications','profile','settings']);
    if (valid.has(explicit)) return explicit;
    if (/soporte|ticket|support/.test(hay)) return 'supportTickets';
    if (/prestamo|préstamo|loan|solicitud/.test(hay)) return 'loanManagement';
    if (/inventario|stock|insumo|asset|barcode/.test(hay)) return 'inventory';
    if (/equipo|team|mentor/.test(hay)) return 'team';
    if (/usuario|sesión|sesion|login|logout|perfil/.test(hay)) return 'users';
  ...
  window.notificationDestinationV46 = routeNotificationV46;
  ...
  selectNotification = async function(id){
    ...
    state.notificationsOpen = false;
    try { await window.sb?.markNotificationRead?.(id); } catch(e) { console.warn('markNotificationRead', e); }
    const destination = routeNotificationV46(target);
    if (typeof openView === 'function') openView(destination); else { state.currentView = destination; renderView?.(); }
    updateHeader?.();
    renderNotificationDropdown?.();
    setTimeout(() => {
      const selector = `[data-id="${(window.CSS && CSS.escape) ? CSS.escape(String(id)) : String(id).replace(/"/g,'\\"')}"]`;
  ...
  document.addEventListener('click', function(e){
    const historyBtn = e.target.closest('#notificationsPreview [data-view-btn], #notificationsPreview .full-width');
    if (historyBtn && historyBtn.closest('#notificationsPreview')) {
      e.preventDefault(); e.stopPropagation();
      state.notificationsOpen = false;
      openView?.(historyBtn.dataset.viewBtn || 'notifications');
      updateHeader?.(); renderNotificationDropdown?.();
      return;
    }
    const row = e.target.closest('#notificationsPreview .notification-select, #notificationsPreview .notification-row[data-id], .notification-card-body[data-id]');
    if (row && row.dataset.id) {
      e.preventDefault(); e.stopPropagation();
      selectNotification(row.dataset.id);
    }
  }, true);

  const previousRenderDropdownV46 = renderNotificationDropdown;
  renderNotificationDropdown = function(){
    if (!notificationsPreview) return;
    const rows = (state.notifications || []).filter(n => (n.unread || !n.read_at)).slice(0,30);
    notificationsPreview.innerHTML = rows.length ? rows.map(n => `<button type="button" class="notification-row notification-select unread" data-id="${escapeAttr(n.id)}"${notificationAccentStyle(n)}><div><strong>${escapeHtml(n.title||'Notificación')}</strong><span>${notificationMessage(n)}</span><small>${notificationMeta(n)}</small></div><span class="status-pill status-pending">Sin leer</span></button>`).join('') + `<button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications">Ver historial</button>` : `<p class="muted">No hay notificaciones sin leer.</p><button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications">Ver historial</button>`;
    notificationsPreview.classList.toggle('open', !!state.notificationsOpen);
  };
  window.renderNotificationDropdown = renderNotificationDropdown;

  showToast = function(title, text='', icon='success'){
    return Swal.fire({toast:true, target: document.body, position:'top-end', timer:2800, timerProgressBar:true, showConfirmButton:false, icon, title, text, heightAuto:false, customClass:{popup:'app-toast-popup', container:'app-toast-container'}, didOpen:(toast)=>{ toast.style.zIndex='2147483647'; }});
  };
  window.showToast = showToast;

  try { updateHeader?.(); renderNotificationDropdown?.(); } catch(e) { console.warn('v46 refresh', e); }
})();

/* === V47: FIX REAL campana, historial, alertas top-end y rutas === */
(function(){
  if (window.__v47RealFixApplied) return; window.__v47RealFixApplied = true;

  function routeNotificationV47(n){
    const explicit = String(n?.section || n?.module || n?.target_view || '').trim();
    const aliases = { supportTicket:'supportTickets', support:'supportTickets', tickets:'supportTickets', loans:'loanManagement', loan:'loanManagement', teams:'team', access:'accessControl' };
    if (aliases[explicit]) return aliases[explicit];
    const valid = new Set(['dashboard','users','roles','team','inventory','loanManagement','supportTickets','accessControl','courses','library','notifications','profile','settings']);
    if (valid.has(explicit)) return explicit;
    const hay = `${explicit} ${n?.title||''} ${n?.message||''}`.toLowerCase();
    if (/soporte|ticket|incidencia|support/.test(hay)) return 'supportTickets';
    if (/prestamo|préstamo|loan|solicitud|devoluci/.test(hay)) return 'loanManagement';
    if (/inventario|stock|insumo|equipo|asset|barcode|código|codigo/.test(hay)) return 'inventory';
    if (/usuario|sesión|sesion|login|acceso|perfil/.test(hay)) return 'users';
    if (/rol|permiso/.test(hay)) return 'roles';
    if (/curso|campus|clase/.test(hay)) return 'courses';
    if (/biblioteca|archivo|documento/.test(hay)) return 'library';
    if (/team|mentor/.test(hay)) return 'team';
    return 'notifications';
  }
  window.notificationDestinationV47 = routeNotificationV47;

  function goViewV47(view){
    const aliases = { supportTicket:'supportTickets', support:'supportTickets', access:'accessControl', loan:'loanManagement' };
    const next = aliases[view] || view || 'notifications';
    state.notificationsOpen = false;
    state.currentView = next;
    try { renderView(); } catch(e) { console.warn('renderView V47', e); }
    try { updateHeader(); renderNotificationDropdown(); } catch(_) {}
    setTimeout(() => {
      const content = document.getElementById('appContent') || document.querySelector('main') || document.body;
      content?.scrollTo?.({top:0, behavior:'smooth'});
    }, 20);
  }
  window.openView = goViewV47;

  selectNotification = async function(id){
    const target = (state.notifications || []).find(n => String(n.id) === String(id));
    if (!target) { goViewV47('notifications'); return; }
    target.unread = false;
    target.read_at = target.read_at || new Date().toISOString();
    try { await window.sb?.markNotificationRead?.(id); } catch(e) { console.warn('markNotificationRead V47', e); }
    goViewV47(routeNotificationV47(target));
  };
  window.selectNotification = selectNotification;

  function handleNotificationPointer(e){
    const preview = document.getElementById('notificationsPreview');
    if (!preview) return;
    const inPreview = e.target.closest && e.target.closest('#notificationsPreview');
    if (!inPreview) return;
    const history = e.target.closest('[data-view-btn="notifications"], .full-width');
    if (history) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.(); goViewV47('notifications'); return; }
    const row = e.target.closest('.notification-select[data-id], .notification-row[data-id], .notification-card-body[data-id]');
    if (row?.dataset?.id) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.(); selectNotification(row.dataset.id); }
  }
  document.addEventListener('pointerdown', handleNotificationPointer, true);
  document.addEventListener('click', handleNotificationPointer, true);

  renderNotificationDropdown = function(){
    const box = document.getElementById('notificationsPreview');
    if (!box) return;
    const seen = new Set();
    const rows = (state.notifications || []).filter(n => (n.unread || !n.read_at)).filter(n => {
      const k = [n.recipient_profile_id||'', n.title||'', n.message||'', n.section||'', String(n.created_at||'').slice(0,16)].join('|');
      if (seen.has(k)) return false; seen.add(k); return true;
    }).slice(0,30);
    box.innerHTML = rows.length
      ? rows.map(n => `<button type="button" class="notification-row notification-select unread" data-id="${escapeAttr(n.id)}"${notificationAccentStyle(n)}><div><strong>${escapeHtml(n.title||'Notificación')}</strong><span>${notificationMessage(n)}</span><small>${notificationMeta(n)}</small></div><span class="status-pill status-pending">Sin leer</span></button>`).join('') + `<button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications">Ver historial</button>`
      : `<p class="muted">No hay notificaciones sin leer.</p><button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications">Ver historial</button>`;
    box.classList.toggle('open', !!state.notificationsOpen);
  };
  window.renderNotificationDropdown = renderNotificationDropdown;

  showToast = function(title, text='', icon='success'){
    if (!window.Swal) return alert([title,text].filter(Boolean).join('\n'));
    return Swal.fire({toast:true, target:document.body, position:'top-end', icon, title, text, timer:3200, timerProgressBar:true, showConfirmButton:false, heightAuto:false, backdrop:false, customClass:{container:'swal2-container-toast-v47', popup:'app-toast-popup app-toast-popup-v47'}, didOpen:(toast)=>{ toast.style.zIndex='2147483647'; }});
  };
  window.showToast = showToast;

  try { renderNotificationDropdown(); updateHeader?.(); } catch(e) { console.warn('V47 init', e); }
})();

/* === V48: header notification routing final + sticky mobile header === */
(function(){
  if (window.__v48HeaderNotificationRoutingApplied) return;
  window.__v48HeaderNotificationRoutingApplied = true;

  const VALID_VIEWS_V48 = new Set(['dashboard','users','roles','team','inventory','loanManagement','supportTickets','accessControl','courses','library','notifications','profile','settings']);
  const VIEW_ALIASES_V48 = {
    supportTicket:'supportTickets', supportTickets:'supportTickets', support:'supportTickets', ticket:'supportTickets', tickets:'supportTickets', soporte:'supportTickets',
    loan:'loanManagement', loans:'loanManagement', prestamos:'loanManagement', préstamo:'loanManagement', prestamo:'loanManagement',
    team:'team', teams:'team', equipo:'team', equipos:'team',
    access:'accessControl', acceso:'accessControl', accessControl:'accessControl',
    inventory:'inventory', inventario:'inventory', users:'users', usuarios:'users', roles:'roles', permisos:'roles',
    courses:'courses', campus:'courses', library:'library', biblioteca:'library', notifications:'notifications', notificaciones:'notifications'
  };

  function normalizeTextV48(value){
    return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  }

  function routeNotificationV48(n){
    const explicitRaw = String(n?.section || n?.module || n?.target_view || n?.targetView || n?.view || '').trim();
    if (VALID_VIEWS_V48.has(explicitRaw)) return explicitRaw;
    const explicit = normalizeTextV48(explicitRaw);
    if (VIEW_ALIASES_V48[explicit]) return VIEW_ALIASES_V48[explicit];

    const hay = normalizeTextV48(`${explicitRaw} ${n?.title || ''} ${n?.message || ''}`);
    if (/\b(soporte|support|ticket|incidencia|falla tecnica|no enciende|estado:|urgencia:)\b/.test(hay)) return 'supportTickets';
    if (/\b(prestamo|prestamos|loan|solicitud|devolucion|retirado|vencido|aprobado)\b/.test(hay)) return 'loanManagement';
    if (/\b(inventario|stock|insumo|asset|barcode|codigo|serie|serial|alta de inventario)\b/.test(hay)) return 'inventory';
    if (/\b(inicio de sesion|sesion|login|logout|usuario|usuarios|perfil)\b/.test(hay)) return 'users';
    if (/\b(rol|roles|permiso|permisos)\b/.test(hay)) return 'roles';
    if (/\b(curso|cursos|campus|clase|docente)\b/.test(hay)) return 'courses';
    if (/\b(biblioteca|archivo|documento|material)\b/.test(hay)) return 'library';
    if (/\b(equipo|equipos|team|mentor)\b/.test(hay)) return 'team';
    return 'notifications';
  }
  window.notificationDestinationV48 = routeNotificationV48;

  function goViewV48(view){
    const key = String(view || 'notifications').trim();
    const alias = VIEW_ALIASES_V48[key] || VIEW_ALIASES_V48[normalizeTextV48(key)] || key;
    const next = VALID_VIEWS_V48.has(alias) ? alias : 'notifications';
    state.notificationsOpen = false;
    state.currentView = next;
    try { renderView(); } catch (e) { console.warn('renderView V48', e); }
    try { updateHeader(); } catch (_) {}
    try { renderNotificationDropdown(); } catch (_) {}
    setTimeout(function(){
      try {
        const scroller = document.querySelector('.content, .app-content, #appContent, main') || document.scrollingElement || document.documentElement;
        if (scroller && typeof scroller.scrollTo === 'function') scroller.scrollTo({ top: 0, behavior: 'smooth' });
        else window.scrollTo({ top: 0, behavior: 'smooth' });
      } catch (_) { window.scrollTo(0, 0); }
    }, 30);
  }
  window.openView = goViewV48;

  selectNotification = async function(id){
    const target = (state.notifications || []).find(function(n){ return String(n.id) === String(id); });
    if (target) {
      target.unread = false;
      target.read_at = target.read_at || new Date().toISOString();
      try { await window.sb?.markNotificationRead?.(id); } catch (e) { console.warn('markNotificationRead V48', e); }
      goViewV48(routeNotificationV48(target));
    } else {
      goViewV48('notifications');
    }
  };
  window.selectNotification = selectNotification;

  function htmlNotificationRowV48(n){
    const view = routeNotificationV48(n);
    return `<button type="button" class="notification-row notification-select unread" data-id="${escapeAttr(n.id)}" data-target-view="${escapeAttr(view)}" onclick="event.preventDefault(); event.stopPropagation(); window.selectNotification && window.selectNotification('${escapeAttr(String(n.id)).replace(/'/g, '&#39;')}'); return false;"${notificationAccentStyle(n)}><div><strong>${escapeHtml(n.title || 'Notificación')}</strong><span>${notificationMessage(n)}</span><small>${notificationMeta(n)}</small></div><span class="status-pill status-pending">Sin leer</span></button>`;
  }
  ...
  renderNotificationDropdown = function(){
    ...
    if (!box) return;
    const seen = new Set();
    const rows = (state.notifications || []).filter(function(n){ return n.unread || !n.read_at; }).filter(function(n){
      const key = [n.recipient_profile_id || '', n.title || '', n.message || '', n.section || '', String(n.created_at || '').slice(0,16)].join('|');
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    }).slice(0, 30);
    box.innerHTML = rows.length
      ? rows.map(htmlNotificationRowV48).join('') + `<button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications" onclick="event.preventDefault(); event.stopPropagation(); window.openView && window.openView('notifications'); return false;">Ver historial</button>`
      : `<p class="muted">No hay notificaciones sin leer.</p><button type="button" class="btn btn-secondary btn-sm full-width" data-view-btn="notifications" onclick="event.preventDefault(); event.stopPropagation(); window.openView && window.openView('notifications'); return false;">Ver historial</button>`;
    box.classList.toggle('open', !!state.notificationsOpen);
  };
  window.renderNotificationDropdown = renderNotificationDropdown;

  function captureHeaderNotificationClickV48(e){
    const path = (typeof e.composedPath === 'function') ? e.composedPath() : [];
    const target = e.target;
    const inPreview = target?.closest?.('#notificationsPreview') || path.some(function(x){ return x && x.id === 'notificationsPreview'; });
    if (!inPreview) return;
    const history = target.closest?.('[data-view-btn="notifications"], .full-width');
    if (history) {
      e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
      goViewV48('notifications');
      return;
    }
    const row = target.closest?.('.notification-select[data-id], .notification-row[data-id], .notification-card-body[data-id]');
    if (row?.dataset?.id) {
      e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
      selectNotification(row.dataset.id);
    }
  ...
  ['click'].forEach(function(type){
    document.addEventListener(type, captureHeaderNotificationClickV48, {capture:true, passive:false});
  });

  try { renderNotificationDropdown(); updateHeader?.(); } catch (e) { console.warn('V48 init', e); }
})();

/* === V49: navegación definitiva de notificaciones + header fixed mobile === */
(function(){
  if (window.__v49HeaderFixApplied) return;
  window.__v49HeaderFixApplied = true;

  const VALID = new Set(['dashboard','users','roles','team','inventory','loanManagement','supportTickets','accessControl','courses','library','notifications','profile','settings']);
  const norm = (v) => String(v || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const aliases = {
    supportticket:'supportTickets', supporttickets:'supportTickets', soporte:'supportTickets', ticket:'supportTickets', tickets:'supportTickets', incidencia:'supportTickets', incidencias:'supportTickets',
    loan:'loanManagement', loans:'loanManagement', prestamo:'loanManagement', prestamos:'loanManagement', devolucion:'loanManagement', devoluciones:'loanManagement',
    inventory:'inventory', inventario:'inventory', stock:'inventory', insumo:'inventory', insumos:'inventory', asset:'inventory', barcode:'inventory',
    user:'users', users:'users', usuario:'users', usuarios:'users', login:'users', sesion:'users', inicio:'users',
    notification:'notifications', notifications:'notifications', notificacion:'notifications', notificaciones:'notifications', historial:'notifications',
    role:'roles', roles:'roles', permisos:'roles', team:'team', teams:'team', equipo:'team', equipos:'team',
    access:'accessControl', accesscontrol:'accessControl', acceso:'accessControl', campus:'courses', cursos:'courses', courses:'courses', biblioteca:'library', library:'library'
  };

  function destinationFromNotification(n){
    const explicit = n?.section || n?.module || n?.target_view || n?.targetView || n?.view || n?.route || '';
    if (VALID.has(String(explicit))) return String(explicit);
    const e = norm(explicit).replace(/[^a-z0-9]/g,'');
    if (aliases[e]) return aliases[e];
    const hay = norm(`${explicit} ${n?.title || ''} ${n?.message || ''}`);
    if (/soporte|support|ticket|incidencia|falla tecnica|no enciende|urgencia|estado:/.test(hay)) return 'supportTickets';
    if (/prestamo|prestamos|loan|devolucion|retirado|vencido|aprobado|solicitud/.test(hay)) return 'loanManagement';
    if (/inventario|ingreso de inventario|cambio de condicion|stock|insumo|asset|barcode|codigo|serie|serial|sf20gm7|netbook/.test(hay)) return 'inventory';
    if (/inicio de sesion|sesion|login|usuario|usuarios|administrador|perfil/.test(hay)) return 'users';
    if (/rol|roles|permiso|permisos/.test(hay)) return 'roles';
    if (/equipo|equipos|team/.test(hay)) return 'team';
    if (/campus|curso|cursos|docente|clase/.test(hay)) return 'courses';
    if (/biblioteca|material|documento|archivo/.test(hay)) return 'library';
    return 'notifications';
  }

  function navigateV49(view){
    const cleaned = norm(view).replace(/[^a-z0-9]/g,'');
    const next = VALID.has(String(view)) ? String(view) : (aliases[cleaned] || 'notifications');
    state.notificationsOpen = false;
    state.currentView = next;
    ...
    requestAnimationFrame(() => {
      const targets = [document.querySelector('.main-area'), document.querySelector('#appContent'), document.scrollingElement, document.documentElement, document.body].filter(Boolean);
      targets.forEach(el => { try { if ('scrollTop' in el) el.scrollTop = 0; } catch(_){} });
      try { window.scrollTo({ top: 0, behavior: 'auto' }); } catch(_) { window.scrollTo(0,0); }
    });
  }
  window.openView = navigateV49;
  window.navigateToView = navigateV49;

  async function selectV49(id){
    const n = (state.notifications || []).find(x => String(x.id) === String(id));
    if (!n) return navigateV49('notifications');
    n.unread = false;
    n.read_at = n.read_at || new Date().toISOString();
    try { await window.sb?.markNotificationRead?.(id); } catch (e) { console.warn('markNotificationRead V49', e); }
    navigateV49(destinationFromNotification(n));
  }
  selectNotification = selectV49;
  window.selectNotification = selectV49;
  window.notificationDestinationV49 = destinationFromNotification;

  const rowHtml = (n) => {
    const view = destinationFromNotification(n);
  ...
  renderNotificationDropdown = function(){
    ...
    box.innerHTML = rows.length
      ? rows.map(rowHtml).join('') + `<button type="button" class="btn btn-secondary btn-sm full-width notification-history-btn" data-target-view="notifications">Ver historial</button>`
      : `<p class="muted">No hay notificaciones sin leer.</p><button type="button" class="btn btn-secondary btn-sm full-width notification-history-btn" data-target-view="notifications">Ver historial</button>`;
    box.classList.toggle('open', !!state.notificationsOpen);
    box.onclick = function(ev){
      const row = ev.target.closest?.('.notification-select[data-id], .notification-row[data-id]');
      const history = ev.target.closest?.('.notification-history-btn, [data-target-view="notifications"], [data-view-btn="notifications"]');
      ev.stopPropagation();
      if (row?.dataset?.id) { ev.preventDefault(); selectV49(row.dataset.id); return false; }
      if (history) { ev.preventDefault(); navigateV49('notifications'); return false; }
    };
  };
  window.renderNotificationDropdown = renderNotificationDropdown;

  function captureClickOnly(ev){
    const inBox = ev.target?.closest?.('#notificationsPreview');
    if (!inBox) return;
    const row = ev.target.closest?.('.notification-select[data-id], .notification-row[data-id]');
    const history = ev.target.closest?.('.notification-history-btn, [data-view-btn="notifications"], [data-target-view="notifications"]');
    if (!row && !history) return;
    ev.preventDefault();
    ev.stopPropagation();
    if (ev.stopImmediatePropagation) ev.stopImmediatePropagation();
    if (row?.dataset?.id) selectV49(row.dataset.id); else navigateV49('notifications');
  }
  document.addEventListener('click', captureClickOnly, { capture:true, passive:false });

  function applyFixedHeader(){
    const isMobile = window.matchMedia('(max-width: 820px)').matches;
    document.documentElement.classList.toggle('v49-mobile-fixed-header', isMobile);
    document.body.classList.toggle('v49-mobile-fixed-header', isMobile);
  }
  applyFixedHeader();
  window.addEventListener('resize', applyFixedHeader, { passive:true });
  window.addEventListener('orientationchange', applyFixedHeader, { passive:true });

  try { renderNotificationDropdown(); updateHeader?.(); } catch(e) { console.warn('V49 init', e); }
})();


/* === V50: ruteo blindado de notificaciones + soporte mobile compacto === */
(function(){
  if (window.__v50FinalFixApplied) return;
  window.__v50FinalFixApplied = true;

  const VALID = new Set(['dashboard','users','roles','team','inventory','loanManagement','supportTickets','accessControl','courses','library','notifications','profile','settings']);
  const normalize = (v) => String(v || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const compact = (v) => normalize(v).replace(/[^a-z0-9]/g,'');
  const aliases = {
    supportticket:'supportTickets', supporttickets:'supportTickets', soporte:'supportTickets', soporteticket:'supportTickets', ticket:'supportTickets', tickets:'supportTickets', incidencia:'supportTickets', incidencias:'supportTickets', fallatecnica:'supportTickets', noenciende:'supportTickets',
    loanmanagement:'loanManagement', prestamo:'loanManagement', prestamos:'loanManagement', loan:'loanManagement', loans:'loanManagement', devolucion:'loanManagement', devoluciones:'loanManagement', solicitud:'loanManagement', solicitudes:'loanManagement',
    inventario:'inventory', inventory:'inventory', stock:'inventory', insumo:'inventory', insumos:'inventory', asset:'inventory', assets:'inventory', barcode:'inventory', codigobarras:'inventory', ingresodeinventario:'inventory', cambiodecondicion:'inventory',
    usuario:'users', usuarios:'users', users:'users', user:'users', login:'users', iniciodesesion:'users', sesion:'users', perfil:'users',
    rol:'roles', roles:'roles', permiso:'roles', permisos:'roles', equipo:'team', equipos:'team', team:'team', teams:'team',
    campus:'courses', curso:'courses', cursos:'courses', courses:'courses', biblioteca:'library', library:'library', notificaciones:'notifications', notifications:'notifications', historial:'notifications'
  };

  function v50Destination(n){
    const explicit = n?.target_view || n?.targetView || n?.view || n?.route || n?.section || n?.module || '';
    if (VALID.has(String(explicit))) return String(explicit);
    const c = compact(explicit);
    if (aliases[c]) return aliases[c];
    const text = normalize(`${explicit} ${n?.title || ''} ${n?.message || ''}`);
    if (/soporte|support|ticket|incidencia|falla tecnica|no enciende|urgencia|cargo:|estado:/.test(text)) return 'supportTickets';
    if (/prestamo|prestamos|loan|solicitud|devolucion|retirado|vencido|aprobado/.test(text)) return 'loanManagement';
    if (/inventario|ingreso de inventario|cambio de condicion|stock|insumo|asset|barcode|codigo|serie|serial|netbook|sf20gm7/.test(text)) return 'inventory';
    if (/inicio de sesion|sesion|login|usuario|usuarios|administrador|perfil/.test(text)) return 'users';
    if (/rol|roles|permiso|permisos/.test(text)) return 'roles';
    if (/equipo|equipos|team/.test(text)) return 'team';
    if (/campus|curso|cursos|docente|clase/.test(text)) return 'courses';
    if (/biblioteca|material|documento|archivo/.test(text)) return 'library';
    return 'notifications';
  }

  function v50Navigate(view){
    const raw = String(view || 'notifications');
    const next = VALID.has(raw) ? raw : (aliases[compact(raw)] || 'notifications');
    state.notificationsOpen = false;
    state.currentView = next;
    try { renderView(); } catch(e) { console.error('renderView V50', e); }
    try { renderNav && renderNav(); } catch(_) {}
    try { updateHeader && updateHeader(); } catch(_) {}
    try { renderNotificationDropdown && renderNotificationDropdown(); } catch(_) {}
    requestAnimationFrame(function(){
      [document.querySelector('.main-area'), document.querySelector('.content'), document.querySelector('#appContent'), document.scrollingElement, document.documentElement, document.body].filter(Boolean).forEach(function(el){ try { el.scrollTop = 0; } catch(_){} });
      try { window.scrollTo(0,0); } catch(_) {}
    });
  }

  async function v50SelectNotification(id){
    const n = (state.notifications || []).find(function(x){ return String(x.id) === String(id); });
    if (!n) return v50Navigate('notifications');
    n.unread = false;
    n.read_at = n.read_at || new Date().toISOString();
    try { await window.sb?.markNotificationRead?.(id); } catch(e) { console.warn('markNotificationRead V50', e); }
    v50Navigate(v50Destination(n));
  }

  window.notificationDestinationV50 = v50Destination;
  window.openView = v50Navigate;
  window.navigateToView = v50Navigate;
  window.selectNotification = v50SelectNotification;
  try { selectNotification = v50SelectNotification; } catch(_) {}

  function rowHtml(n){
    const id = escapeAttr(String(n.id || ''));
    const view = escapeAttr(v50Destination(n));
  ...
  renderNotificationDropdown = function(){
    ...
    box.innerHTML = rows.length ? rows.map(rowHtml).join('') + `<button type="button" class="btn btn-secondary btn-sm full-width notification-history-btn" data-target-view="notifications">Ver historial</button>` : `<p class="muted">No hay notificaciones sin leer.</p><button type="button" class="btn btn-secondary btn-sm full-width notification-history-btn" data-target-view="notifications">Ver historial</button>`;
    box.classList.toggle('open', !!state.notificationsOpen);
  };
  window.renderNotificationDropdown = renderNotificationDropdown;

  function v50NotificationPointer(ev){
    const box = ev.target?.closest?.('#notificationsPreview');
    if (!box) return;
    const row = ev.target.closest?.('.notification-select[data-id], .notification-row[data-id]');
    const history = ev.target.closest?.('.notification-history-btn, [data-view-btn="notifications"]');
    if (!row && !history) return;
    ev.preventDefault();
    ev.stopPropagation();
    if (ev.stopImmediatePropagation) ev.stopImmediatePropagation();
    if (row?.dataset?.id) v50SelectNotification(row.dataset.id);
    else v50Navigate('notifications');
  }
  ['click','pointerup','mouseup'].forEach(function(type){ document.addEventListener(type, v50NotificationPointer, {capture:true, passive:false}); });

  try { renderNotificationDropdown(); updateHeader && updateHeader(); } catch(e) { console.warn('V50 init', e); }
})();
