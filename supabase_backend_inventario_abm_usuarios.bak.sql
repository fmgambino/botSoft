...
-- ===== PATCH V6.4 Usuarios, avatar persistente y ABM Roles =====
-- Permite roles personalizados además de administrator/teacher/student.
alter table public.roles drop constraint if exists roles_code_check;

-- Borra firmas viejas que pudieron quedar cacheadas con role_id bigint/uuid.
...
grant execute on function public.admin_update_profile_full(uuid,text,text,text,text,boolean,text) to authenticated;
...
begin
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='permissions' and policyname='permissions_read_all') then
    create policy permissions_read_all on public.permissions for select to authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='role_permissions' and policyname='role_permissions_read_all') then
    create policy role_permissions_read_all on public.role_permissions for select to authenticated using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='roles' and policyname='roles_admin_all') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='role_permissions' and policyname='role_permissions_admin_all') then
...
-- =========================================================
-- PATCH v6.5: Usuarios completos + eliminación real desde ABM
-- Ejecutar después de versiones anteriores.
-- =========================================================
...
begin
  ...
  set full_name = coalesce(nullif(p_full_name,''), full_name),
      ...
      title = case when p_role_code = 'student' then null else nullif(p_title,'') end,
...
begin
  ...
  update public.inventory_loans set requester_id = null, approved_by = null where requester_id = p_profile_id or approved_by = p_profile_id;
  update public.inventory_transactions set actor_id = null, related_profile_id = null where actor_id = p_profile_id or related_profile_id = p_profile_id;
...
-- =========================
-- PATCH V6.6 - Usuarios y equipos
-- =========================

alter table if exists public.teams add column if not exists project text;
alter table if exists public.teams add column if not exists mentor_backup_id uuid references public.profiles(id) on delete set null;

-- Reemplaza firma vieja que declaraba role_id como bigint.
...
begin
  ...
  insert into public.profiles (id, role_id, full_name, dni, whatsapp, birth_date, title, is_active, avatar_url)
  values (v_profile_id, v_role_id, coalesce(nullif(p_full_name,''), split_part(p_email,'@',1)), nullif(p_dni,''), nullif(p_whatsapp,''), p_birth_date, case when p_role_code='student' then null else nullif(p_title,'') end, coalesce(p_is_active,true), coalesce(nullif(p_avatar_url,''),'./assets/avatar-default.svg'))
  on conflict (id) do update set
    role_id = excluded.role_id,
    full_name = excluded.full_name,
    dni = excluded.dni,
...
set search_path = public
as $$
  select t.id, t.name, coalesce(t.project,'') as project, coalesce(t.description,'') as description,
         t.teacher_id as mentor_id, t.mentor_backup_id,
         coalesce(array_agg(tm.profile_id) filter (where rp.code='student'), '{}') as student_ids,
         array_remove(array[mentor.full_name, backup.full_name], null) as teachers,
         coalesce(array_agg(distinct c.name) filter (where c.name is not null), '{}') as courses,
         coalesce(array_agg(distinct d.name) filter (where d.name is not null), '{}') as divisions,
         count(tm.profile_id) filter (where rp.code='student')::int as students
  from public.teams t
  ...
  left join public.roles rp on rp.id = sp.role_id
  left join public.courses c on c.id = sp.student_course_id
  left join public.divisions d on d.id = sp.student_division_id
  group by t.id, mentor.full_name, backup.full_name
...
begin
  if not public.is_admin(auth.uid()) then
    raise exception 'Solo administradores pueden gestionar equipos';
  end if;
  if p_mentor_id is null then raise exception 'Debe seleccionar un mentor docente'; end if;
  if p_mentor_backup_id is not null and p_mentor_backup_id = p_mentor_id then raise exception 'El mentor suplente debe ser otro docente'; end if;
  select count(*) into v_count from unnest(coalesce(p_student_ids,'{}'::uuid[]));
  if v_count > 5 then raise exception 'Cada equipo puede tener hasta 5 alumnos'; end if;

  insert into public.teams(id, name, description, project, teacher_id, mentor_backup_id, status)
  values(coalesce(p_team_id, gen_random_uuid()), p_name, p_description, p_project, p_mentor_id, p_mentor_backup_id, 'activo')
  on conflict(id) do update set name=excluded.name, description=excluded.description, project=excluded.project, teacher_id=excluded.teacher_id, mentor_backup_id=excluded.mentor_backup_id, updated_at=now()
  returning id into v_team_id;

  delete from public.team_members where team_id = v_team_id;
  insert into public.team_members(team_id, profile_id, is_leader)
  select v_team_id, p_mentor_id, true
  where p_mentor_id is not null;
  insert into public.team_members(team_id, profile_id, is_leader)
  select v_team_id, p_mentor_backup_id, false
  where p_mentor_backup_id is not null;
  insert into public.team_members(team_id, profile_id, is_leader)
  select v_team_id, x, false from unnest(coalesce(p_student_ids,'{}'::uuid[])) x;
  return v_team_id;
end;
$$;
...
as $$
begin
  if not public.is_admin(auth.uid()) then raise exception 'Solo administradores pueden eliminar equipos'; end if;
...
-- =========================
-- V6.7 PATCH INVENTARIO OPTIMIZADO
-- Ejecutar después del esquema base si ya existe la base.
-- Agrega Marca, Empresa/Proveedor, Locación física, Zona, importación CSV y barcodes imprimibles.
-- =========================
...
insert into public.inventory_locations (code, name, address) values
('LAB-ROB', 'Laboratorio de Robótica', 'Instituto San Miguel'),
('DEP-01', 'Depósito', 'Instituto San Miguel'),
('TALLER', 'Taller revisión', 'Instituto San Miguel')
...
begin
  ...
  values (p_name, v_category_id, v_brand_id, v_supplier_id, case when lower(coalesce(p_category,'')) like '%insumo%' then 'consumible' else 'serializado' end, nullif(p_location_detail,''), nullif(p_zone,''))
  returning id into v_catalog_id;

  select id into v_location_id from public.inventory_locations where code = coalesce(nullif(p_location_code,''),'LAB-ROB') limit 1;
  select id into v_status_id from public.inventory_status_catalog where code = 'nuevo' limit 1;

  v_barcode := coalesce(nullif(p_barcode,''), public.generate_inventory_barcode('ISMROB'));
  v_asset_code := coalesce(nullif(p_asset_code,''), 'ACT-' || replace(v_barcode, 'ISMROB-', ''));

  insert into public.inventory_assets(catalog_item_id, asset_code, barcode, serial_number, generated_barcode, status_id, current_location_id, condition_note, location_detail, zone)
  values (v_catalog_id, v_asset_code, v_barcode, nullif(p_serial_number,''), nullif(p_barcode,'') is null, v_status_id, v_location_id, p_condition_note, nullif(p_location_detail,''), nullif(p_zone,''))
...
begin
  ...
  if nullif(p_category,'') is not null then
    insert into public.inventory_categories(name) values (p_category)
      on conflict (name) do update set name=excluded.name returning id into v_category_id;
  end if;
  if nullif(p_brand,'') is not null then
    insert into public.inventory_brands(name) values (p_brand)
  ...
  update public.inventory_catalog_items ci
  set name = coalesce(nullif(p_name,''), ci.name),
      ...
      default_location_detail = coalesce(nullif(p_location_detail,''), ci.default_location_detail),
      default_zone = coalesce(nullif(p_zone,''), ci.default_zone),
      updated_at = now()
  where ci.id = v_asset.catalog_item_id;
